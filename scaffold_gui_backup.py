...
Design:  SpaceX-inspired dark theme with mana-master SVG icons
Icons:   C:/Temp/mana-master/svg  (optional — graceful fallback)
Usage:   python scaffold_gui.py
Requires:  pip install customtkinter
Optional:  pip install Pillow cairosvg   (for mana symbol icons)
"""
...
# ── Optional image libraries for mana icons ───────────────────────────────
_HAS_PIL = False
_HAS_CAIROSVG = False
try:
...
# ─────────────────────────────────────────────────────────────────────────────
# Mana icon directory + font discovery
# ─────────────────────────────────────────────────────────────────────────────
_MANA_ICON_DIR: Path | None = None
_MANA_FONT_PATH: Path | None = None
for _candidate_base in [
    os.environ.get("MANA_ICON_DIR", ""),
    str(Path("C:/Temp/mana-master")),
    str(Path(__file__).resolve().parent.parent / "mana-master"),
    str(Path(__file__).resolve().parent / "mana-master"),
]:
    if not _candidate_base:
        continue
    _base = Path(_candidate_base)
    # Check for svg/ subdirectory or direct svg dir
    _svg_dir = _base / "svg" if (_base / "svg").is_dir() else (
        _base if _base.is_dir() and (_base / "w.svg").exists() else None)
    if _svg_dir and not _MANA_ICON_DIR:
        _MANA_ICON_DIR = _svg_dir
    # Check for fonts/ subdirectory
    _font_file = _base / "fonts" / "mana.ttf"
    if _font_file.exists() and not _MANA_FONT_PATH:
        _MANA_FONT_PATH = _font_file
    if _MANA_ICON_DIR and _MANA_FONT_PATH:
        break

# Mana TTF icon font — comprehensive character mapping (Unicode PUA)
_MANA_FONT_CHARS: dict[str, str] = {
    # ── Mana colours ──────────────────────────────────────────────────────
    "W": "\ue600",  "U": "\ue601",  "B": "\ue602",
    "R": "\ue603",  "G": "\ue604",
    # ── Card types ────────────────────────────────────────────────────────
    ...
    "flying": "\ue952",        "deathtouch": "\ue94b",
    "menace": "\ue95d",        "flash": "\ue951",
    "counter": "\ue954",       "removal": "\uea69",  # exile
    "wipe": "\ue9a7",          "bounce": "\ue965",    # transform
    "protection": "\ue954",    "hexproof": "\ue954",
    "draw": "\ue948",          "ramp": "\ue622",      # land
    "tutor": "\ue94f",         "mill": "\ue940",      # skull
    "etb": "\ue966",           "lifegain": "\uea4b",  # lifelink
    "pump": "\ue93d",          "ward": "\ue992",
    ...
    "counter_time": "\ue942",  "counter_loyalty": "\ue937",
    "power": "\ue921",         "toughness": "\ue922",
    "rarity": "\ue96c",        "tap": "\ue61a",
    "loyalty_up": "\ue627",    "loyalty_down": "\ue625",
    "loyalty_start": "\ue628",
    "colorpie": "\ue9f0",      "multiple": "\ue925",
    "spark": "\ue909",         "infinity": "\ue903",
    # ── Guilds (2-colour) ────────────────────────────────────────────────
    ...
    # ── Clans / Shards (3-colour) ────────────────────────────────────────
    "clan_abzan": "\ue916",    "clan_jeskai": "\ue917",
    "clan_mardu": "\ue918",    "clan_sultai": "\ue919",
    "clan_temur": "\ue91a",
}

# Map guild/clan names → icon keys for deck name preview
_GUILD_ICON_MAP: dict[str, str] = {
    # Guilds (2-colour)
    ...
    # Clans / Wedges (3-colour with icon)
    "Abzan": "clan_abzan",        "Jeskai": "clan_jeskai",
    "Mardu": "clan_mardu",        "Sultai": "clan_sultai",
    "Temur": "clan_temur",
    # Shards (3-colour, no dedicated icon — use generic multi)
    "Bant": "multiple",           "Esper": "multiple",
    "Grixis": "multiple",         "Jund": "multiple",
    "Naya": "multiple",
    # Mono-colours — use mana colour icon
    "Mono-White": "W",            "Mono-Blue": "U",
    "Mono-Black": "B",            "Mono-Red": "R",
    "Mono-Green": "G",
    # Five-Color — colorpie icon
    "Five-Color": "colorpie",
}

# Map archetype group names → icon keys for section headers
_ARCHETYPE_GROUP_ICONS: dict[str, str] = {
    "Aggro": "haste",
    "Midrange": "creature",
    "Control": "instant",
    "Combo": "sorcery",
    "Graveyard": "counter_skull",
    "Permanents": "enchantment",
    "Ramp & Lands": "land",
}

# Map tag category names → icon keys
_TAG_CATEGORY_ICONS: dict[str, str] = {
    "Offensive": "power",
    "Defensive": "counter_shield",
    "Utility": "draw",
}

# ─────────────────────────────────────────────────────────────────────────────
# Modern Dark Dashboard Palette
# ─────────────────────────────────────────────────────────────────────────────
APP_TITLE = "MTG Deck Scaffold Generator"
WIN_W, WIN_H = 960, 1000

ACCENT       = "#c9a227"          # gold — thematic for MTG
ACCENT_HOVER = "#b08d1f"
ACCENT_DIM   = "#2a2210"
BG           = "#0f1117"          # deep dark navy
CARD_BG      = "#171c28"          # elevated card surface
CARD_BORDER  = "#1e2538"          # subtle visible border
SURFACE      = "#1a1f2e"          # form inputs, interactives
SURFACE_ALT  = "#252d3d"          # hover state
BORDER       = "#2a3346"          # visible but muted
TEXT         = "#e8eaf0"          # soft white
TEXT_DIM     = "#8b95a8"          # readable secondary
TEXT_MUTED   = "#4a5568"          # tertiary / hints
SUCCESS      = "#34d399"
ERROR        = "#f87171"
WARNING      = "#fbbf24"
INFO_BLUE    = "#60a5fa"
DIVIDER      = "#1e2538"          # section separators

MANA_COLORS = {
    "W": {"bg": "#f9f4e0", "fg": "#1a1a1a", "dim": "#2a2720",
           "glow": "#f9f4e0", "label": "W", "icon_fg": "#f9f4e0"},
    "U": {"bg": "#2b7acc", "fg": "#ffffff", "dim": "#142640",
           "glow": "#2b7acc", "label": "U", "icon_fg": "#5cb3ff"},
    "B": {"bg": "#7b5e99", "fg": "#e0d4ee", "dim": "#221a30",
           "glow": "#7b5e99", "label": "B", "icon_fg": "#c4a8e0"},
    "R": {"bg": "#d44035", "fg": "#ffffff", "dim": "#301414",
           "glow": "#d44035", "label": "R", "icon_fg": "#ff7b7b"},
    "G": {"bg": "#2a9e50", "fg": "#ffffff", "dim": "#132e1e",
           "glow": "#2a9e50", "label": "G", "icon_fg": "#6ade8a"},
}

# ── Guild presets — click to auto-select colours ──────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Mana icon rendering — TTF font (primary) → cairosvg SVG → letter fallback
# ─────────────────────────────────────────────────────────────────────────────
_mana_icon_cache: dict[str, "ctk.CTkImage | None"] = {}
_mana_font_obj: "ImageFont.FreeTypeFont | None" = None
_mana_font_loaded: bool = False
...
def _get_mana_font(size: int) -> "ImageFont.FreeTypeFont | None":
    """Load mana.ttf icon font at requested pixel size. Cached."""
    global _mana_font_obj, _mana_font_loaded
    if _mana_font_loaded:
        # Re-create at new size if needed (PIL fonts are size-specific)
        if _MANA_FONT_PATH and _HAS_PIL:
            try:
                return ImageFont.truetype(str(_MANA_FONT_PATH), size)
            except Exception:
                return None
        return None
    _mana_font_loaded = True
    if _MANA_FONT_PATH and _HAS_PIL:
        try:
            _mana_font_obj = ImageFont.truetype(str(_MANA_FONT_PATH), size)
            return _mana_font_obj
        except Exception:
            pass
    return None


def _draw_circle_bg(draw, size, circle_bg, mc, selected):
    """Draw the circular background + optional glow ring."""
    bg_rgb = _hex_to_rgb(circle_bg)
    alpha = 255 if selected else 200
    draw.ellipse([1, 1, size - 2, size - 2],
                 fill=(*bg_rgb, alpha))
    if selected:
        glow_rgb = _hex_to_rgb(mc["glow"])
        draw.ellipse([0, 0, size - 1, size - 1],
                     outline=(*glow_rgb, 255), width=2)


def _render_mana_icon(color_key: str, size: int = 32,
                      selected: bool = False) -> "ctk.CTkImage | None":
    """Render a mana symbol icon. Returns CTkImage or None.

    Rendering priority:
    1. mana.ttf icon font + PIL  (primary — works everywhere Pillow exists)
    2. cairosvg + SVG files       (best quality, needs native cairo lib)
    3. PIL circle + letter         (final fallback)
    """
    cache_key = f"{color_key}_{size}_{selected}"
    if cache_key in _mana_icon_cache:
        return _mana_icon_cache[cache_key]

    if not _HAS_PIL:
        return None

    mc = MANA_COLORS.get(color_key)
    if not mc:
        return None

    icon_fg = mc["icon_fg"] if not selected else mc["fg"]
    circle_bg = mc["bg"] if selected else mc["dim"]
    fg_rgb = _hex_to_rgb(icon_fg)

    # ── Tier 1: mana.ttf icon font ───────────────────────────────────────
    glyph_char = _MANA_FONT_CHARS.get(color_key)
    font_size = int(size * 0.52)
    mana_font = _get_mana_font(font_size) if glyph_char else None

    if mana_font and glyph_char:
        try:
            canvas = Image.new("RGBA", (size, size), (0, 0, 0, 0))
            draw = ImageDraw.Draw(canvas)
            _draw_circle_bg(draw, size, circle_bg, mc, selected)

            # Render the mana glyph character centered
            bbox = draw.textbbox((0, 0), glyph_char, font=mana_font)
            tw, th = bbox[2] - bbox[0], bbox[3] - bbox[1]
            tx = (size - tw) // 2 - bbox[0]
            ty = (size - th) // 2 - bbox[1]
            draw.text((tx, ty), glyph_char,
                      fill=(*fg_rgb, 255), font=mana_font)

            result = ctk.CTkImage(light_image=canvas, dark_image=canvas,
                                  size=(size, size))
            _mana_icon_cache[cache_key] = result
            return result
        except Exception:
            pass

    # ── Tier 2: cairosvg SVG rendering ────────────────────────────────────
    svg_path = (_MANA_ICON_DIR / f"{color_key.lower()}.svg"
                if _MANA_ICON_DIR else None)
    if svg_path and svg_path.exists() and _HAS_CAIROSVG:
        try:
            svg_data = svg_path.read_text(encoding="utf-8")
            svg_data = svg_data.replace('fill="#444"',
                                        f'fill="{icon_fg}"')
            icon_sz = int(size * 0.58)
            png_data = cairosvg.svg2png(
                bytestring=svg_data.encode("utf-8"),
                output_width=icon_sz, output_height=icon_sz)
            symbol = Image.open(io.BytesIO(png_data)).convert("RGBA")

            canvas = Image.new("RGBA", (size, size), (0, 0, 0, 0))
            draw = ImageDraw.Draw(canvas)
            _draw_circle_bg(draw, size, circle_bg, mc, selected)

            offset_x = (size - icon_sz) // 2
            offset_y = (size - icon_sz) // 2
            canvas.paste(symbol, (offset_x, offset_y), symbol)
    ...
    # ── Tier 3: PIL circle + letter fallback ──────────────────────────────
    try:
        canvas = Image.new("RGBA", (size, size), (0, 0, 0, 0))
        draw = ImageDraw.Draw(canvas)
        _draw_circle_bg(draw, size, circle_bg, mc, selected)
        ...
        _mana_icon_cache[cache_key] = None
        return None


def _clear_icon_cache():
    """Clear the mana icon cache (called on selection changes)."""
    _mana_icon_cache.clear()


_font_icon_cache: dict[str, "ctk.CTkImage | None"] = {}


def _render_font_icon(icon_key: str, size: int = 16,
                      color: str = TEXT_DIM) -> "ctk.CTkImage | None":
    """Render any icon from mana.ttf by key. Returns CTkImage or None."""
    cache_key = f"fi_{icon_key}_{size}_{color}"
    if cache_key in _font_icon_cache:
        return _font_icon_cache[cache_key]
    if not _HAS_PIL:
        return None
    glyph = _MANA_FONT_CHARS.get(icon_key)
    if not glyph:
        return None
    font = _get_mana_font(size)
    if not font:
        return None
    try:
        ...
        tx = (size - tw) // 2 - bbox[0]
        ty = (size - th) // 2 - bbox[1]
        draw.text((tx, ty), glyph, fill=(*fg_rgb, 255), font=font)
        result = ctk.CTkImage(light_image=canvas, dark_image=canvas,
                               size=(size, size))
        _font_icon_cache[cache_key] = result
        return result
    except Exception:
        _font_icon_cache[cache_key] = None
        return None


# ─────────────────────────────────────────────────────────────────────────────
# Tooltip — clean SpaceX-style hover
# ─────────────────────────────────────────────────────────────────────────────
class Tooltip:
...
# ─────────────────────────────────────────────────────────────────────────────
# Typography — SpaceX: clean geometric sans-serif, generous sizing
# ─────────────────────────────────────────────────────────────────────────────
_F_BODY = _F_SMALL = _F_BOLD = _F_MONO = None
_F_TITLE = _F_SECTION = _F_HINT = _F_CAPS = None


def _init_fonts():
    global _F_BODY, _F_SMALL, _F_BOLD, _F_MONO
    global _F_TITLE, _F_SECTION, _F_HINT, _F_CAPS
    _F_BODY    = ctk.CTkFont(size=13)
    _F_SMALL   = ctk.CTkFont(size=11)
    _F_BOLD    = ctk.CTkFont(size=13, weight="bold")
    _F_MONO    = ctk.CTkFont(family="Cascadia Code", size=11)
    _F_TITLE   = ctk.CTkFont(size=18, weight="bold")
    _F_SECTION = ctk.CTkFont(size=13, weight="bold")
    _F_HINT    = ctk.CTkFont(size=11)
    _F_CAPS    = ctk.CTkFont(size=10, weight="bold")


# ── SpaceX-styled widget factories ────────────────────────────────────────
...
def w_button(parent, text, command=None, *, primary=False, **kw):
    if primary:
        d = dict(fg_color=ACCENT, hover_color=ACCENT_HOVER, text_color=BG,
                 font=_F_BOLD, height=40, corner_radius=6, border_width=0)
    else:
        d = dict(fg_color="transparent", hover_color=SURFACE_ALT,
...
def _section_label(parent, text):
    """SpaceX-style uppercase section label with tracking."""
    return ctk.CTkLabel(parent, text=text.upper(),
                        font=_F_CAPS, text_color=TEXT_MUTED)
...
class ScaffoldApp(ctk.CTk):
    def __init__(self):
        ...
        self._mana_icons: dict[str, dict] = {}   # color -> {btn, icons}
        self._build_ui()
    ...
    def _build_ui(self):
        # ── Header bar — modern dashboard ─────────────────────────────────
        hdr = ctk.CTkFrame(self, fg_color=CARD_BG, corner_radius=0,
                            height=54)
        hdr.pack(fill="x")
        hdr.pack_propagate(False)
        # Thin gold accent line at top
        ctk.CTkFrame(hdr, fg_color=ACCENT, height=2,
                      corner_radius=0).pack(fill="x", side="top")
        hdr_inner = ctk.CTkFrame(hdr, fg_color="transparent")
        hdr_inner.pack(fill="both", expand=True)
        # App title icon + name
        title_icon = _render_font_icon("planeswalker", 18, ACCENT)
        if title_icon:
            ctk.CTkLabel(hdr_inner, text="", image=title_icon,
                         width=20).pack(side="left", padx=(24, 6), pady=14)
        ctk.CTkLabel(hdr_inner, text=APP_TITLE,
                     font=ctk.CTkFont(size=14, weight="bold"),
                     text_color=TEXT).pack(side="left",
                                           padx=(0 if title_icon else 24, 0),
                                           pady=14)
        w_button(hdr_inner, "\u2193 Load", self._on_load_settings,
                 width=70, height=28).pack(side="right", padx=(4, 20), pady=13)
        w_button(hdr_inner, "\u2191 Save", self._on_save_settings,
                 width=70, height=28).pack(side="right", padx=0, pady=13)
    ...
    # ── Card container — modern dashboard with subtle elevation ───────────

    def _card(self, parent):
        c = ctk.CTkFrame(parent, fg_color=CARD_BG, corner_radius=14,
                          border_color=CARD_BORDER, border_width=1)
        c.pack(fill="x", padx=10, pady=CARD_PAD_Y)
        return c

    def _card_header(self, card, number, title, hint=""):
        """Modern section header — number badge + title."""
        row = ctk.CTkFrame(card, fg_color="transparent")
        row.pack(fill="x", padx=INNER_PAD, pady=(18, 4 if hint else 14))
        # Number badge — small rounded rectangle
        ctk.CTkLabel(row, text=number,
                     font=ctk.CTkFont(size=10, weight="bold"),
                     text_color=ACCENT, fg_color=ACCENT_DIM,
    ...
    def _divider(self, parent):
        ...
        ctk.CTkFrame(parent, fg_color=DIVIDER, height=1,
                      corner_radius=0).pack(fill="x", padx=INNER_PAD, pady=4)
    ...
    def _build_scaffold_tab(self):
        ...
        self.focus_box = ctk.CTkTextbox(
            ...
            border_width=1, corner_radius=6, height=80, wrap="word")
        self.focus_box.pack(fill="x", padx=INNER_PAD, pady=(0, 16))

        # Card 6: Deck Name (with auto-generate)
        c6 = self._card(self.scroll)
        self._card_header(c6, "06", "Deck Name")
        self._build_deck_name(c6)

        # Card 7: Options
        c7 = self._card(self.scroll)
        self._card_header(c7, "07", "Options")
        self._build_options(c7)

        # Card 8: Output Directory
        c8 = self._card(self.scroll)
        self._card_header(c8, "08", "Output Directory",
                          "Default: Decks/")
        self._build_output(c8)

    # ── Card 1: Mana Colours — icon-based circular buttons ───────────────

    def _build_colors(self, p):
        f = ctk.CTkFrame(p, fg_color="transparent")
        f.pack(fill="x", padx=INNER_PAD, pady=(0, 8))
        self._color_btns = {}
        self._color_frames = {}
        ...
        for c in COLOR_ORDER:
            ...
            self._color_frames[c] = col_frame

            # Try to get mana icon
            icon = _render_mana_icon(c, size=40, selected=False)

            if icon:
                btn = ctk.CTkButton(
                    col_frame, text="", image=icon,
                    width=56, height=56, corner_radius=28,
                    fg_color=mc["dim"], hover_color=SURFACE_ALT,
                    border_color=BORDER, border_width=1,
                    command=lambda col=c: self._toggle_color(col))
            else:
                # Text fallback — clean circular button
                btn = ctk.CTkButton(
                    col_frame, text=mc["label"],
                    width=56, height=56, corner_radius=28,
                    fg_color=mc["dim"], hover_color=SURFACE_ALT,
                    text_color=mc["icon_fg"], border_color=BORDER,
                    border_width=1,
                    font=ctk.CTkFont(size=18, weight="bold"),
                    command=lambda col=c: self._toggle_color(col))
            btn.pack()

            # Mana name below — small, muted
            ctk.CTkLabel(col_frame, text=MANA_NAMES[c],
                         font=ctk.CTkFont(size=10),
                         text_color=TEXT_MUTED).pack(pady=(4, 0))
            self._color_btns[c] = btn

        # ── Guild / Clan indicator row ────────────────────────────────────
        self._guild_row = ctk.CTkFrame(p, fg_color="transparent")
        self._guild_row_visible = False
        self._guild_display_icon = ctk.CTkLabel(
            self._guild_row, text="", width=0)
        self._guild_display_icon.pack(side="left", padx=(0, 0))
        self._guild_display_icon_active = False
        self._guild_display_label = ctk.CTkLabel(
            self._guild_row, text="",
            font=ctk.CTkFont(size=13, weight="bold"),
            text_color=ACCENT)
        self._guild_display_label.pack(side="left")

        # ── Guild preset buttons — click to auto-select colours ───────────
        self._divider(p)
        preset_hdr = ctk.CTkFrame(p, fg_color="transparent")
        preset_hdr.pack(fill="x", padx=INNER_PAD, pady=(4, 6))
        _section_label(preset_hdr, "Guild Presets").pack(side="left")
        # Scrollable row of guild/clan preset buttons
        preset_scroll = ctk.CTkScrollableFrame(
            p, fg_color="transparent", height=68,
            orientation="horizontal",
            scrollbar_button_color=BORDER,
            scrollbar_button_hover_color=ACCENT)
        preset_scroll.pack(fill="x", padx=INNER_PAD, pady=(0, 14))
        for guild_name, colors in GUILD_PRESETS.items():
            icon_key = _GUILD_ICON_MAP.get(guild_name)
            icon = _render_font_icon(icon_key, 18, TEXT_DIM) \
                if icon_key else None
            preset_btn = ctk.CTkButton(
                preset_scroll,
                text=guild_name,
                image=icon if icon else None,
                compound="top" if icon else "center",
                width=72, height=56, corner_radius=10,
                fg_color=SURFACE, hover_color=SURFACE_ALT,
                text_color=TEXT_DIM, border_color=BORDER,
                border_width=1,
                font=ctk.CTkFont(size=9),
                command=lambda g=guild_name: self._apply_guild_preset(g))
            preset_btn.pack(side="left", padx=(0, 4))
    ...
    def _toggle_color(self, c):
        ...
        if c in self.selected_colors:
            ...
            icon = _render_mana_icon(c, size=40, selected=False)
            if icon:
                btn.configure(image=icon, fg_color=mc["dim"],
                              border_color=BORDER, border_width=1)
            else:
                btn.configure(fg_color=mc["dim"], text_color=mc["icon_fg"],
                              border_color=BORDER, border_width=1)
        else:
            self.selected_colors.add(c)
            icon = _render_mana_icon(c, size=40, selected=True)
            if icon:
                btn.configure(image=icon, fg_color=mc["bg"],
                              border_color=mc["glow"], border_width=2)
            else:
                btn.configure(fg_color=mc["bg"], text_color=mc["fg"],
                              border_color=mc["glow"], border_width=2)
        self._update_guild_display()
        self._update_auto_name()
        self._validate_live()

    def _update_guild_display(self):
        """Update the guild/clan indicator when colour selection changes."""
        color_key = frozenset(self.selected_colors)
        color_name = GUILD_NAMES.get(color_key, "")
        guild_icon_key = _GUILD_ICON_MAP.get(color_name)

        if color_name and len(self.selected_colors) >= 2:
            # Show the guild row
            if not self._guild_row_visible:
                self._guild_row.pack(fill="x", padx=INNER_PAD, pady=(4, 12))
                self._guild_row_visible = True

            # Update guild icon
            icon = _render_font_icon(guild_icon_key, 22, ACCENT) \
                if guild_icon_key else None
            if icon:
                self._guild_display_icon.configure(image=icon, width=26)
                if not self._guild_display_icon_active:
                    self._guild_display_icon.pack(side="left", padx=(0, 6))
                    self._guild_display_icon_active = True
            elif self._guild_display_icon_active:
                self._guild_display_icon.pack_forget()
                self._guild_display_icon_active = False

            self._guild_display_label.configure(text=color_name)
        else:
            # Hide the guild row
            if self._guild_row_visible:
                self._guild_row.pack_forget()
                self._guild_row_visible = False

    def _apply_guild_preset(self, guild_name: str):
        """Auto-select colours for a guild/clan preset."""
        colors = GUILD_PRESETS.get(guild_name, "")
        if not colors:
            return
        # Clear all current colours first
        for c in list(self.selected_colors):
            self._toggle_color(c)
        # Select the preset colours
        for c in colors:
            if c not in self.selected_colors:
                self._toggle_color(c)

    # ── Card 2: Archetypes — clean grid ──────────────────────────────────

    def _build_archetypes(self, p):
        ct = ctk.CTkFrame(p, fg_color="transparent")
        ct.pack(fill="x", padx=INNER_PAD, pady=(0, 16))
        self._arch_btns = {}
        self._arch_tooltips = {}
        for gn, archs in ARCHETYPE_GROUPS.items():
            # Group header with icon
            h = ctk.CTkFrame(ct, fg_color="transparent")
            h.pack(fill="x", pady=(10, 4))
            grp_icon_key = _ARCHETYPE_GROUP_ICONS.get(gn)
            grp_icon = _render_font_icon(grp_icon_key, 14, ACCENT) \
                if grp_icon_key else None
            if grp_icon:
                ctk.CTkLabel(h, text="", image=grp_icon,
                             width=16).pack(side="left", padx=(0, 6))
            else:
                ctk.CTkFrame(h, fg_color=ACCENT, width=2, height=12,
                             corner_radius=1).pack(side="left", padx=(0, 8))
            _section_label(h, gn).pack(side="left")
            # Button grid
            ...
            for i, (a, desc) in enumerate(archs.items()):
                ...
                btn = ctk.CTkButton(
                    g, text=lbl, height=32, corner_radius=6,
    ...
    def _tribe_search(self):
        ...
        for t in matches[:10]:
            ...
            ctk.CTkButton(
                ...
                fg_color=ACCENT_DIM if already else "transparent",
                hover_color=SURFACE_ALT,
                text_color=ACCENT if already else TEXT_DIM,
    ...
    def _refresh_tribe_chips(self):
        ...
        for t in self._tribes:
            ...
            ctk.CTkLabel(chip, text=t, font=ctk.CTkFont(size=11),
    ...
    # ── Card 4: Tags — SpaceX pill buttons ────────────────────────────────

    def _build_tags(self, p):
        auto_row = ctk.CTkFrame(p, fg_color="transparent")
        auto_row.pack(fill="x", padx=INNER_PAD, pady=(0, 10))
        ...
        for cat_name, cat_tags in TAG_CATEGORIES.items():
            ...
            cat_hdr.pack(fill="x", padx=INNER_PAD, pady=(6, 2))
            cat_icon_key = _TAG_CATEGORY_ICONS.get(cat_name)
            cat_icon = _render_font_icon(cat_icon_key, 12, TEXT_MUTED) \
                if cat_icon_key else None
            if cat_icon:
                ctk.CTkLabel(cat_hdr, text="", image=cat_icon,
                             width=14).pack(side="left", padx=(0, 4))
            _section_label(cat_hdr, cat_name).pack(side="left")
            f = ctk.CTkFrame(p, fg_color="transparent")
            ...
            for i, (tag, desc) in enumerate(cat_tags.items()):
                # Prepend ability icon to tag label
                tag_icon = _render_font_icon(tag, 14, TEXT_MUTED)
                btn = ctk.CTkButton(
                    f, text=tag, height=28, corner_radius=14,
                    image=tag_icon if tag_icon else None,
                    compound="left" if tag_icon else "center",
                    fg_color="transparent", hover_color=SURFACE_ALT,
                    text_color=TEXT_MUTED, border_color=BORDER, border_width=1,
                    font=ctk.CTkFont(size=10),
    ...
    def _build_deck_name(self, p):
        ...
        # Guild/clan icon placeholder
        self._guild_icon_label = ctk.CTkLabel(
            self._name_preview_frame, text="", width=0)
        self._guild_icon_label.pack(side="left", padx=(0, 0))
        self._guild_icon_active = False
    ...
    def _update_auto_name(self):
        ...
        # Update guild/clan icon in preview
        color_key = frozenset(self.selected_colors)
        color_name = GUILD_NAMES.get(color_key, "")
        guild_icon_key = _GUILD_ICON_MAP.get(color_name)
        icon = _render_font_icon(guild_icon_key, 18, ACCENT) \
            if guild_icon_key else None
        if icon:
            self._guild_icon_label.configure(image=icon, width=22)
            self._guild_icon_label.pack(side="left", padx=(0, 4))
            self._guild_icon_active = True
        elif self._guild_icon_active:
            self._guild_icon_label.pack_forget()
            self._guild_icon_active = False
        self.name_entry.configure(state="normal")
        self.name_entry.delete(0, "end")
        if name:
    ...
    def _build_options(self, p):
        f = ctk.CTkFrame(p, fg_color="transparent")
        f.pack(fill="x", padx=INNER_PAD, pady=(0, 16))
        for v, t, kw in [
    ...
    def _build_output(self, p):
        ...
        r.pack(fill="x", padx=INNER_PAD, pady=(0, 16))
    ...
    def _build_queries_tab(self):
        ...
        hdr.pack(fill="x", padx=INNER_PAD, pady=(16, 4))
        ctk.CTkFrame(hdr, fg_color=ACCENT, width=3, height=16,
                      corner_radius=1).pack(side="left", padx=(0, 10))
        ctk.CTkLabel(hdr, text="RUN PENDING SESSION QUERIES",
                     font=_F_SECTION, text_color=ACCENT
                     ).pack(side="left")
        ...
        self.rq_btn.pack(anchor="w", padx=INNER_PAD, pady=(0, 16))
    ...
    def _build_synergy_tab(self):
        ...
        ctk.CTkFrame(hdr, fg_color=ACCENT, width=3, height=16,
                      corner_radius=1).pack(side="left", padx=(0, 10))
        ctk.CTkLabel(hdr, text="GATE 2.5 \u2014 SYNERGY ANALYSIS",
                     font=_F_SECTION, text_color=ACCENT
                     ).pack(side="left")
        ...
        ctk.CTkLabel(sr, text="THRESHOLD", font=_F_CAPS,
        ...
        ctk.CTkLabel(sr, text="MODE", font=_F_CAPS,
                     text_color=TEXT_MUTED).pack(side="left", padx=(0, 6))
        ...
        self.syn_btn.pack(anchor="w", padx=INNER_PAD, pady=(0, 16))

    # ── Log panel — SpaceX mission log style ──────────────────────────────

    def _build_log_panel(self):
        self._log_visible = False
        self._log_tags = set()
        # Thin divider
        ctk.CTkFrame(self, fg_color=DIVIDER, height=1,
        ...
        self._log_toggle = ctk.CTkButton(
            ...
            hover_color=SURFACE_ALT, anchor="w", height=32, corner_radius=0,
        ...
        self._log_frame = ctk.CTkFrame(
            self, fg_color=BG, corner_radius=0, height=0)
        self._log_frame.pack(fill="x")
        self._log_frame.pack_propagate(False)
        self._log_box = ctk.CTkTextbox(
            self._log_frame, fg_color=BG, text_color=TEXT_DIM,
    ...
    # ── Footer — SpaceX mission control status bar ────────────────────────

    def _build_footer(self):
        ctk.CTkFrame(self, height=1, fg_color=DIVIDER,
                      corner_radius=0).pack(fill="x")
        ft = ctk.CTkFrame(self, fg_color=CARD_BG, corner_radius=0, height=66)
        ...
        self._arch_count.pack(anchor="w", pady=(12, 0))
