...
def _score_oracle_crossref_indexed(
    ...
    *oracle_seen* tracks which (card, partner) oracle-TRIGGERS have already
    ...
    if a_subtypes_in_b and name_b not in oracle_seen.get(name_a, set()):
        ...
        _apply_weighted(scores, name_a, name_b, InteractionType.TRIGGERS,
                        primary_axes, confidence="oracle")
    ...
    if b_subtypes_in_a and name_a not in oracle_seen.get(name_b, set()):
        ...
        _apply_weighted(scores, name_b, name_a, InteractionType.TRIGGERS,
                        primary_axes, confidence="oracle")
...
def score_pairwise(
    ...
    """
    # Apply caller-supplied weights globally for this scoring run
    if weights is not None:
        import synergy_types as _st
        _st._DEFAULT_WEIGHTS = weights
    profiles: Dict[str, SynergyProfile] = {}
