...
sys.path.insert(0, str(_here))                        # scripts/analysis/
sys.path.insert(0, str(_here.parent / "utils"))       # scripts/utils/
sys.path.insert(0, str(_here.parent / "cli"))         # scripts/cli/
sys.path.insert(0, str(_here.parent))                 # scripts/ (fallback)
...
def main() -> None:
    ...
    print("Scoring pairwise synergies "
          ...
          file=sys.stderr)
    # Build CompositeWeights from CLI args (previously parsed but never applied)
    weights = CompositeWeights(
        engine_density=args.weight_engine_density,
        synergy_density=args.weight_synergy_density,
        raw_interactions_cap=args.weight_raw_interactions_cap,
        role_breadth=args.weight_role_breadth,
        oracle_confirmed=args.weight_oracle_confirmed,
    )
    ...
    if args.mythic_panel:
        try:
            ...
            # Append to markdown report file if one was written
            if args.output and args.report_format != "json":
                with open(args.output, "a", encoding="utf-8") as f:
                    f.write(panel_md)
                print("Mythic Panel appended to report.", file=sys.stderr)
            else:
                print(panel_md)
            # Always write companion JSON
            json_path = (
                (Path(args.output).parent if args.output else input_path.parent)
                / "panel_report.json"
            )
            json_path.write_text(
                __import__("json").dumps(panel_result, indent=2, default=str),
                encoding="utf-8",
            )
            print(f"Panel JSON written to {json_path}", file=sys.stderr)
        except Exception as _panel_err:
            print(f"[WARN] Mythic Panel failed: {_panel_err}", file=sys.stderr)
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--tribe", default="",
                   help="Tribe name for TribalChief panel evaluation")
